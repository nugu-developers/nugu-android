...
class NudgeDirectiveHandler(private val nudgeInfoObserver: NudgeInfoObserver) : AbstractDirectiveHandler() {
    ...
    data class Payload(
        ...
        @SerializedName("expectSpeechExist")
        var expectSpeechExist: Boolean?,
        @SerializedName("speakTTSExist")
        var speakTTSExist: Boolean?,
        @SerializedName("displayTemplateExist")
        var displayTemplateExist: Boolean?
    )

    private val config: Map<NamespaceAndName, BlockingPolicy> by lazy {
        HashMap<NamespaceAndName, BlockingPolicy>().apply { this[APPEND] = BlockingPolicy() }
    }
    ...
    override fun handleDirective(info: DirectiveInfo) {
        ...
        if (payload == null) {
            ...
            nudgeInfoObserver.onNudgeAppendDirective(info.directive.header.dialogRequestId, payload)
        }
    }
