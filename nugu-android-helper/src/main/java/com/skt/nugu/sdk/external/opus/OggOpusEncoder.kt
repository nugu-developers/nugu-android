...
class OggOpusEncoder : Encoder {
    ...
    private fun createNativeEncoder(): PointerByReference? {
        val error = IntBuffer.allocate(1)
        val encoder =
            JNAOpus.INSTANCE.opus_encoder_create(SAMPLE_RATE, CHANNELS, JNAOpus.OPUS_APPLICATION_VOIP, error)
        ...
        ByteArrayOutputStream().use { os ->
            ByteArrayInputStream(input, offset, size).use { ios ->
                ...
                while (ios.read(data).also { read = it } > 0) {
                    val shortBuffer = ShortBuffer.allocate(read / 2)
                    var opus_encoded: Int
                    var i = 0
                    while (i < read) {
                        opus_encoded = data[i].toInt() and 255
                        val b2: Int = data[i + 1].toInt() shl 8
                        shortBuffer.put((opus_encoded or b2).toShort())
                        i += 2
                    }
                    shortBuffer.flip()
                    val opusEncodedBuffer = ByteBuffer.allocate(read)
                    opus_encoded =
                        JNAOpus.INSTANCE.opus_encode(
                            opusEncoder,
                            shortBuffer,
                            frameSize,
                            opusEncodedBuffer,
    ...
    private fun close() {
        Logger.d(TAG, "[close]")
        try {
            JNAOpus.INSTANCE.opus_encoder_destroy(opusEncoder)
            opusEncoder = null
            writer = null
            frameSize = -1
