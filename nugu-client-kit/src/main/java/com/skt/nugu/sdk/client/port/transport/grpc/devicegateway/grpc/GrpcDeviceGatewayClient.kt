...
/**
 *  Implementation of DeviceGateway with GRPC
 **/
internal class GrpcDeviceGatewayClient(private val policyResponse: PolicyResponse,
    ...
    companion object {
        private const val TAG = "GrpcDeviceGatewayClient"
    ...
    override fun connect(): Boolean {
        ...
        policy.apply {
            ...
            currentChannel = ChannelBuilderUtils.createChannelBuilderWith(option, authDelegate).build()
            currentChannel.also {
                pingService =
                    PingService(
                        VoiceServiceGrpc.newBlockingStub(it),
                        healthCheckPolicy,
                        observer = this@GrpcDeviceGatewayClient
                    )
                eventStreamService =
                    EventStreamService(
                        VoiceServiceGrpc.newStub(it),
                        observer = this@GrpcDeviceGatewayClient
                    )
                crashReportService =
                    CrashReportService(
